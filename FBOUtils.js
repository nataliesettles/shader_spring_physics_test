...
THREE.FBOUtils = function( textureWidth, renderer, simulationShader, attributes ) {
 ...
 //    var positions = new Float32Array( (textureWidth * textureWidth) * 3 );

	// for ( var j = 0, j3 = 0, l = textureWidth * textureWidth; j < l; j ++, j3 += 3 ) {

	// 	positions[ j3 + 0 ] = ( j % textureWidth ) / textureWidth ;
	// 	positions[ j3 + 1 ]  = Math.floor( j / textureWidth ) / textureWidth;
	// 	positions[ j3 + 2 ]  = 0;

	// }

	// plane.addAttribute( 'position', new THREE.BufferAttribute( positions, 3 ) );

	// if ( Object.keys(attributes).length > 0 ) {
	// 	var keys = Object.keys(attributes);
	// 	for ( var i = 0; i < keys.length; i++ ) {
	// 		console.log( keys[i] );
	// 		console.log( attributes[ keys[i] ] );
 //            plane.addAttribute( keys[i], attributes[ keys[i] ] );
	// 	}
	// }
